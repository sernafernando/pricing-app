...
- [ ] PR3.T4 RED: `mark_failed` — after 5 attempts, order is `poisoned`, excluded from claim, counted by `queue.py::poisoned_count(db)` (the health endpoint in PR6 reuses it); failure isolated per-order via savepoint (batch of 199 succeeds despite 1 bad order).
