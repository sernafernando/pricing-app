...
- [ ] PR3.T6d RED (design D5 batch wall-time bound): a batch exceeding `batch_timeout` because one order is slow ⇒ whole batch rolled back, claims released via `release_uncharged` with `attempts` unchanged for all, each order then retried alone; only the slow order, failing alone, is charged (`last_error='timeout'`); all others are recomputed and deleted.
