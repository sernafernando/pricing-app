...
- [x] PR11.T1 RED: `SalesFilter` gains `include_unknown`, `include_mixed`, `include_in_dispute`, `include_provisional` (defaults per R11: unknown OFF, in_dispute OFF, mixed ON, provisional ON) — `build_scope` applies each as an exclusion when OFF (KPI R9, R10, R11).
- [x] PR11.T2 GREEN: extend `filters.py`. Implemented as `_group_switch_subquery` (one aggregate per GROUP, seller-scoped like `members_base`) + `_apply_switches`, joined into both `listing_query` and `facet_base` (table/KPI parity, R10). **Deviation discovered via RED, not assumed**: applying R11's defaults inside `build_scope` silently changed `GET /sales`'s existing behavior for every caller that omits the new params (a bare `paid` order with no shipment has `goods_status='unknown'` by definition, so it was excluded by the new default-OFF "A revisar" switch) — broke 65 pre-existing router/filter tests. Resolved by giving `GET /sales`'s own query params a `True` (show everything) default, distinct from `SalesFilter`'s dataclass defaults (which stay R11-shaped, for the KPI endpoint and future FE callers); documented inline in both places. R11's actual first-load UX defaults are a FE URL-state decision, deferred to PR16.
- [x] PR11.T3 RED: Mixta resolution — collapsed `operation_status='mixed' OR goods_status='mixed'`; `modo_logistico='mixed'` explicitly does NOT count (parity test vs `_collapse`, per design D12 resolution). Verified by mutation (loosening the distinct-count threshold in `is_mixed` made the test fail for the right reason).
- [x] PR11.T4 RED: `aggregate_order_metrics` — `groups_count`, `orders_count`, `gross_billed` (ARS; other currencies counted separately), `neto_sum`+`unknown_count`, `total_gauss_sum`+ok/provisional/unresolved counts, `markup_weighted_pct = SUM(tg)/SUM(costo)*100` (never average of %), `recalculating_count`, excluding recalculating orders from all sums (KPI R8, SM R3). Also excludes `pending`/`failed` orders from every sum (reported via `pending_count`/`failed_count`), reading `neto`/`total_gauss` from `order_metrics.read` only, never a live recompute.
- [x] PR11.T5 GREEN: `backend/app/services/ml_sales_query/aggregate.py`. Verified by mutation (loosening the zero-costo markup guard, and collapsing the recalculating/pending state branches, both broke the right tests).
- [x] PR11.T6 RED: `GET /sales/kpis` — per-toggle excluded counts (KPI R13); `recalculating_count`, `pending_count` (orders with no metrics row: excluded from every sum, counted, never summed as NULL — SM R2, R3) and `worker_alive` fields (derived from worker health). `worker_alive` extracted from `ml_order_metrics.py`'s inline heartbeat check into `order_metrics.health.worker_alive`, reused by both endpoints (no behavior change, confirmed by the existing health-endpoint suite staying green).
- [x] PR11.T7 GREEN: `ml_ventas_ops.py::sales_kpis` endpoint. This endpoint's OWN query-param defaults ARE spec R11 (no legacy caller to break), unlike `GET /sales` (see T2 deviation note).
- [x] PR11.T8 RED: parity test — `TestKpiListingParity` in `tests/integration/test_ml_ventas_ops_kpis_router.py`: 5 switch combinations (not the full 16 — a representative spread covering every toggle both ways) + a search-term case + a non-empty-queue case, asserting `/sales/kpis.orders_count == /sales.total - recalculating_count - pending_count`. Verified by mutation: removing the pending-state exclusion branch from `aggregate_order_metrics` broke both the unit-level `test_aggregate.py` suite AND this parity suite, confirming it discriminates the real code path.
- [x] PR11.T9 RED: toggle state round-trips through URL query params (KPI R12) — `TestUrlParamRoundTrip` (backend param parsing + `effective_switches` echo); full FE round-trip covered in PR16.

### PR11 post-review fixes (pre-merge, same slice)

- [x] PR11.K0 **Binding product decision (user, verbatim: "sí, tienen que entrar, esas son ventas reales")**: an order with NO `ml_shipments_ops` row (in-person pickup / "acordar con el vendedor") was silently classified `goods_status='unknown'` and excluded by the default-OFF "A revisar" switch, under-reporting `gross_billed_ars`. Fixed at the classification level, not by flipping `include_unknown`'s default (which would also hide genuinely doubtful orders): `filters.py::_goods_status_for_switch_expr` treats `MlOrdersOps.shipping_id IS NULL AND has_no_shipping_tag IS TRUE` (ML's own tag, `mode_resolution.has_no_shipping_tag`) as a distinct `'no_shipping'` value, never `'unknown'`, for the doubtful-case switch computation ONLY — the displayed listing/detail `goods_status` badge is unchanged. `has_no_shipping_tag` is nullable; NULL (not tagged) still falls through to `'unknown'`. RED/GREEN + mutation in `test_filters_switches.py::TestIncludeUnknown` and `test_ml_ventas_ops_kpis_router.py::TestNoShippingTagIncludedByDefault`.
- [x] PR11.K1 **Money, blocking**: `markup_weighted_pct`'s numerator (`SUM(total_gauss)`) and denominator (`SUM(costo_mercaderia)`) were summed under two SEPARATE `if` guards in `aggregate.py`, so an order could contribute to one side without the other — a ratio between two different populations. Fixed: an order contributes to EITHER side only when it carries BOTH values; skipped orders are counted via the new `AggregateResult.markup_skipped_count` / `SalesKpiResponse.markup_skipped_count` (never hidden), consistent with the existing `neto_unknown_count` pattern. `total_gauss_sum` (a separate KPI R8 measure) is unaffected. RED/GREEN + mutation (re-split the guard) in `test_aggregate.py::TestWeightedMarkupOnlyFromOrdersCarryingBothValues`.
- [x] PR11.K2 Design D12's "explicit facet selection overrides its switch" was not implemented: an explicit `operation_status=unknown`/`in_dispute` or `goods_status=unknown` filter fought its own toggle, landing the user on an empty table with no explanation. Added `filters.py::effective_switches` (single source of the override rule), applied inside `build_scope` (so `GET /sales` and `GET /sales/kpis` stay in parity, spec R10) and echoed by `sales_kpis`'s `effective_switches` response field instead of the raw request params. RED/GREEN + mutation in `test_filters_switches.py::TestExplicitFacetOverridesSwitch` and `test_ml_ventas_ops_kpis_router.py::TestExplicitFacetOverridesSwitch`.
- [x] PR11.K3 `_toggle_excluded_counts` re-ran the full `build_scope` scope once per OFF toggle (up to 5 full query executions on a 77k-order production table). Replaced with `filters.py::excluded_by_toggle_counts`: ONE aggregate query over `SalesScope.pre_switch_listing_query` (new field: the listing pre-switch-filter, everything else already applied) joined once against `_group_switch_subquery`, using five `COUNT(DISTINCT CASE WHEN ...)` expressions (current + one per toggle flipped) — total round trips for the whole `/sales/kpis` endpoint dropped from up to 8 (3 for `aggregate_order_metrics` + up to 5 for excluded counts) to 4 (3 + 1). RED/GREEN + mutation (reverted to the per-toggle re-run) in `test_ml_ventas_ops_kpis_router.py::TestExcludedByToggleQueryShape` (bounded via the shared `query_counter` fixture).
- [x] PR11.K4 `is_unknown`'s single-value check (`op_single == "unknown"` / `goods_single == "unknown"`, `op_single`/`goods_single` being `MIN()` over the group's members) only worked because every status name in today's vocabulary sorts alphabetically BEFORE `'unknown'` — a genuinely mixed group never had `MIN()` picking `'unknown'` out of a multi-value set, by coincidence, not by design. Replaced with `filters.py::_switch_flags`'s explicit "this axis collapsed to 0 or 1 distinct value, and that value is 'unknown'" check, mirroring `collapse()`'s real semantics — a mixed group is now caught by `is_mixed` alone, never spuriously also by `is_unknown`. RED/GREEN + mutation (monkeypatching a shipping-status mapping to a synthetic status sorting after `'unknown'`) in `test_filters_switches.py::TestUnknownClassificationIsExplicitNotAlphabetical`.
- [x] PR11.K5 Docstrings updated across `filters.py` (`build_scope`, `_goods_status_expr`/new `_goods_status_for_switch_expr`, `_group_switch_subquery`, `SalesScope`), `aggregate.py` (module docstring, `AggregateResult`), and `ml_ventas_ops.py::sales_kpis` to reflect K0-K4 above; the stale "switches NOT applied yet" note in `build_scope`'s docstring (left over from before PR11.T2 landed) is also corrected.
...
- [x] PR13.T1 RED (Vitest+RTL): `VentasMLLayout` — CSS grid `minmax(0,1fr) var(--ventas-panel-width)` when a row is selected, single column otherwise; panel is a sticky `<aside aria-label="Detalle de venta">`, no overlay/`aria-modal`, `focus trap (PANEL R16, R20).
- [x] PR13.T2 GREEN: implement frontend/src/components/ventasMl/VentasMLLayout.jsx` + CSS module (tokens only, light+dark).
- [x] PR13.T3 RED: selection state driven by URL param (`orden`); Escape clears it and closes the panel without trapping focus; screen-reader live-region announces panel content changes (PANEL R19, R20).
...
- [x] PR13.T11 Delete `frontend/src/components/DesgloseDrawer.jsx` (+css/test) only after PR13.T9 migration is verified green.
