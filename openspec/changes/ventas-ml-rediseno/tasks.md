...
- [ ] PR3.T6b RED: a slow-but-alive batch whose total duration exceeds the 120 s lease (lease and intervals scaled down in the test) completes with ZERO `attempts` charged to any of its orders and no `lease_expired` entries; no other claimer can take its rows meanwhile.
