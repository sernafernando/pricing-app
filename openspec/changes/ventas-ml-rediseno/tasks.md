...
- [ ] PR9.T16a RED: CSV parsing of `marcas`, `subcategorias`, `pms` — non-numeric ids, empty entries (`"1,,2"`), duplicates and an unknown id are handled explicitly (reject with HTTP 422 (spec scenario 12)), never silently turning into "no filter" and never a 500.
