...
**Choice**: table
`ml_order_metrics_dirty(order_id BIGINT PK, version BIGINT NOT NULL DEFAULT 1, reason VARCHAR(32) NOT NULL, enqueued_at TIMESTAMPTZ NOT NULL DEFAULT now(), claimed_at TIMESTAMPTZ NULL, claimed_by VARCHAR(64) NULL, claim_token UUID NULL, attempts SMALLINT NOT NULL DEFAULT 0, last_error TEXT NULL)` + index on (enqueued_at).
...
`INSERT ... SELECT unnest(ids) ON CONFLICT (order_id) DO UPDATE SET version = dirty.version + 1, enqueued_at = now(), reason = EXCLUDED.reason, attempts = 0, last_error = NULL` then `PERFORM pg_notify('order_metrics_dirty', '')`. It does NOT touch `claimed_at`, `claimed_by`, `claim_token`: an in-flight claim stays owned by its worker, the version bump alone makes that worker's release keep the row dirty (D5 fence). Resetting `attempts`/`last_error` is intentional: a new input write is a new chance, so this upsert un-parks a previously poisoned order the moment its inputs change again.
...
5. Heartbeat (rev 3, time-based, independent of drain length): `ctx.heartbeat()` upserts `worker_job_state` row `name='worker'` (`heartbeat_at`, pid, host, `detail = {draining: bool, drained_in_run: int}`) in its own short block, rate-limited to at most once per 5 s. It is called every loop iteration AND by every handler after each batch (the drain calls it between batches AND between orders inside a batch; reconcile and divergence between their slices). The call is cheap (a clock check; it writes only when ≥ 5 s elapsed). A drain of any length therefore refreshes the heartbeat at least every `max(5 s, one order's recompute)`, and one order is bounded by the per-statement timeout (D5), so the heartbeat never ages past the 30 s threshold while the worker is alive. `worker_alive` = `now() - heartbeat_at < 30 s`, nothing else — a long backfill drain (PR6) never reads as a dead worker. Health also exposes `worker_draining` from `detail.draining`.
...
**Attempts rule (precise).** `attempts` increments in exactly two cases: (a) an exception raised while recomputing that order (Failure below); (b) its claim is found with an EXPIRED lease and no release — the worker died (OOM, segfault, SIGKILL) or hung past the lease holding it. It NEVER increments on a normal claim or on a version-changed release (a lost race is a re-dirty, not a failure). At `attempts >= 5` the row is parked: excluded from claim, counted in `poisoned_count`. To keep an innocent order from being charged for a batch-mate that killed the process, a row with `attempts > 0` is claimed in a batch of ONE (a separate claim query `WHERE claimed_at IS NULL AND attempts BETWEEN 1 AND 4 ... LIMIT 1` runs before the normal batch, which only takes `attempts = 0`), so a process-killing order isolates itself and parks after 5 expired leases while the rest of the queue keeps draining. Hangs become exceptions: every recompute block sets `SET LOCAL statement_timeout = '30s'` and the drain stops starting new orders after `ctx.deadline` (= claim time + lease/2), so a live worker always releases well before its lease expires and case (b) only fires for a dead process.
...
- Health: `GET /api/ml-ops/order-metrics/health` (perm `ml_ops.ver`): `{queue_depth, oldest_dirty_age_s, claimed_count, poisoned_count, worker_heartbeat_at, worker_alive (heartbeat < 30 s, refreshed per batch — see D4 step 5), worker_draining, listener_mode ('notify'|'poll_only'), last_divergence: {run_at, divergent_count, missing_count}, formula_version}`. Worker down ⇒ queue grows, oldest age grows, heartbeat stale — visible in the endpoint and the KPI strip banner ("Recálculo detenido" when worker_alive=false and queue_depth>0).
...
## Data Flow
    ...
    pricing-worker (systemd)  LISTEN via DATABASE_URL_DIRECT  | safety poll 5s | startup drain
        ...
        |   not owner: rollback to savepoint (no stale write) -> commit ; heartbeat (<= every 5 s)
        v
