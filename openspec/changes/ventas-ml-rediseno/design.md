...
`filters.py`'s `SalesFilter` gains `marcas`, `subcategorias`, `pms`, `tienda_oficial`. All join through the frozen cost snapshot `ml_order_item_costos.producto_item_id`, never `ProductoPricing`:
...
- **Official store** (`tienda_oficial`): `productos_erp.item_id` → `tb_mercadolibre_items_publicados.item_id` → `parsear_tiendas_oficiales_mla` / `build_filtro_tiendas_oficiales_mla` (imported from `productos_shared.py`, NOT reimplemented) — same store definition and `sin_tienda` token semantics as the Productos screen (spec R36).
