...
**No-op writes never enqueue (rev 5).** Postgres fires an `UPDATE OF col` trigger whenever `col` is in the SET list, even if its value did not change, and the sweep and the payment ingestion re-upsert the same rows every cycle. Every UPDATE trigger therefore carries a change guard: row-level triggers use `WHEN (OLD.col1 IS DISTINCT FROM NEW.col1 OR OLD.col2 IS DISTINCT FROM NEW.col2 ...)` over exactly the formula's read columns of that table; statement-level triggers join `OLD TABLE` and `NEW TABLE` on the key and enqueue only rows where a read column IS DISTINCT FROM its old value. Without this, every sweep would re-enqueue (and, through the input-write reset, un-park) every touched order: needless load, and a process-killing order could never stay parked.

**Postgres trigger restrictions (rev 7).** Transition tables (`REFERENCING OLD TABLE / NEW TABLE`) are not allowed on a trigger with a column list, and a statement-level trigger's WHEN cannot reference row values. So every STATEMENT-level trigger is declared on the whole event (e.g. `AFTER UPDATE ON transportes`, `AFTER INSERT OR UPDATE OR DELETE ON configuracion`) with no column list and no WHEN, and does its column filtering (`cp` IS DISTINCT FROM its old value) and key filtering (`clave IN ('lluvia_offset_tipo','lluvia_offset_valor')`) inside the function over the transition tables. Column-scoped `UPDATE OF` plus a row-level WHEN guard remain valid for ROW-level triggers.
...
2. **Recompute, then fenced store per order** (rev 6: two phases, so no row lock outlives one order's write). (a) COMPUTE: in a read-only block with no row locks, compute the whole batch in bulk from committed inputs (`compute_order_metrics`), under `batch_timeout` and `statement_timeout`. (b) STORE: for EACH order, its OWN short transaction: `SELECT version FROM ml_order_metrics_dirty WHERE order_id = :id AND claim_token = :token FOR UPDATE`. No row → this worker no longer owns the claim (lease expired and another worker reclaimed, or row deleted) → ROLLBACK, nothing written. Row found → upsert ml_order_metrics + ml_venta_deducciones + legacy columns, then: version unchanged → `DELETE` the dirty row; version changed (an input write landed meanwhile) → `UPDATE ... SET claimed_at = NULL, claimed_by = NULL, claim_token = NULL` so the row is re-claimed next pass. COMMIT. Row locks are released at each order's commit (Postgres holds row locks until the transaction ends; a savepoint rollback does not release them, which is why one batch-wide transaction was wrong), so a writer's trigger upsert and the heartbeat's lease renewal wait at most for ONE order's store, never for the batch. Because the compute phase holds no locks, a `BatchTimeout` there rolls back nothing that others wait on.
...
- `metrics_state` per order: 'failed' if its dirty row is parked (attempts >= 5; wins), else 'recalculating' if a dirty row exists (wins over stored status), else stored gauss_status, else 'pending' (no metrics row yet). A failed order is never shown as recalculating forever: it gets its own badge ("Falló el recálculo"), is excluded from KPI sums and counted as `failed_count`, and is listed in the health endpoint with its last error.
- KPI: orders in the queue are excluded from all sums and reported as `recalculating_count`; orders with no `ml_order_metrics` row yet (`pending`, spec R2) and parked orders (`failed`) are ALSO excluded and reported separately as `pending_count` and `failed_count` (never summed as NULL, never silently dropped); UI shows "N recalculando" chip. Never a stale number as good.
...
- KPI response carries `recalculating_count`, `pending_count` and `worker_alive` so the screen itself surfaces a dead worker.
...
- GATE: readers-switch PR merges only after prod health shows the backlog drained (queue_depth counting only claimable rows = 0), divergent_count=0 and missing_count=0 on a run, re-checked over a monitoring window. `missing_count` counts orders with no metrics row EXCLUDING parked ones; parked orders (attempts >= 5) are reported separately as `poisoned_count` with their order_ids and `last_error` (health lists them). Parked orders never block the gate by themselves, but the user must review that list and explicitly accept it when confirming the gate — nothing is hidden, and one broken order cannot block the whole change. A parked order that later gets an input write is un-parked (D3) and recomputed normally.
...
### D12a — Product-level sale filters (NEW, spec `ml-sales-product-filters`, user binding decision 2026-09-22)

**Facet conjunction (user decision 2026-09-22): ONE item must satisfy ALL active facets.** Not one EXISTS per facet (that would let different items satisfy different facets), but a SINGLE correlated EXISTS over the group's order-items carrying every active facet condition, matching what the Productos screen means by combining filters.
`filters.py`'s `SalesFilter` gains `marcas`, `subcategorias`, `pms`. All join through the frozen cost snapshot `ml_order_item_costos.producto_item_id`, never `ProductoPricing`:
- **Path**: `ml_orders_ops`/pack group → `ml_order_item_costos` (order_id match; `producto_item_id` join target) → `productos_erp` (marca, subcategoria_id) for `marcas`/`subcategorias`; `pms` resolves via `marcas_pm.usuario_id → (marca, categoria)` pairs then the same `productos_erp` join, mirroring `productos_listing.py::listar_productos`'s `pms` branch verbatim (same pair-resolution query, no second implementation).
- **Official store: OUT (user decision 2026-09-22).** It could only be read from the product's CURRENT publications, so the same sale would match or stop matching as publications move — today's catalog, not the sale. Same class as publication status.
- **Publication status: OUT (verified).** `tb_mercadolibre_items_publicados` has no MLA column (only `item_id`), so Productos' `estado_mla` is a PRODUCT-level "has at least one active publication today" filter (productos_listing.py ~835-850). On a sales screen that describes today's catalog, not the sale, so it is out of scope with the rest of the current-state filters.
- **Group (pack) semantics (spec R38)**: identical to the existing promo group semantics — filter selects GROUPS (a pack, or a lone-order group) through a SINGLE correlated EXISTS over the group's member order-items that carries EVERY active facet condition: one item must satisfy all of them at once (never one EXISTS per facet OR'd together, which would let different items satisfy different facets). A matching item returns the WHOLE group.
- **Unresolved items (spec R39)**: an order-item with no `ml_order_item_costos` row (LEFT JOIN, `producto_item_id IS NULL`) is excluded from the `EXISTS` match for every product-level filter, but never removed from the base row set by that fact alone — it only fails to contribute a match; an unrelated filter, or no filter at all, still returns its sale/pack normally.
- **Out of scope (spec R40)**: stock, con_precio, web_transferencia, Tienda Nube, colores, con/sin MLA presence, nuevos_ultimos_7_dias, auditoría — all describe the product's state today, not the sale, and are explicitly not ported.
- **Frozen-only markup (spec R41)**: if a markup/rebate/oferta-style filter is added to this screen, it MUST read `ml_order_item_costos.costo_unitario_ars` / `precio_unitario` / `iva_pct` (the frozen snapshot), never `ProductoPricing`; no such filter ships in this change unless it can be expressed that way.
- **Promo/PxQ deferred (spec R42)**: no promo/PxQ param is added to `SalesFilter` in this change; the mlwebhook cross-DB promo family only reflects current promo state, not the sale's historical promo state, and reconciling that gap is out of scope.
- **Index (spec R43, own earlier PR)**: `ml_order_item_costos.producto_item_id` has NO existing index (verified: only `order_id`, `item_id`, `id` are indexed on that model) and every filter above joins through it. A new index `ix_ml_order_item_costos_producto_item_id` ships as its OWN Alembic migration (`YYYYMMDD_ix_ml_order_item_costos_producto_item_id.py`, single head, downgrade drops it), landing BEFORE the PR that adds these joins (PR9, see tasks.md).
- **Query-count / plan note**: the listing (`GET /sales`) pages at most 200 rows per request, so its product-filter joins run once per page against an already-narrowed, LIMIT-bounded query — cost is dominated by the base `ml_orders_ops` scope, not by these joins. The KPI aggregate (`GET /sales/kpis`) has no LIMIT: it aggregates the WHOLE filtered set in one query, so the same joins run over every matching row once (still O(1) queries — one `EXISTS`/join per active filter, no per-row loop). Both endpoints share `build_scope`, so their EXPLAIN shape only differs by the LIMIT; an `EXPLAIN ANALYZE` capture for a representative filter combination (brand + subcategoría + date range) is a PR9 review artifact confirming the new index is used (`Index Scan` / `Bitmap Index Scan` on `ix_ml_order_item_costos_producto_item_id`, not a sequential scan) for both shapes.
- **Same-database note**: pricing and sales share the same PostgreSQL database (unlike the promo family, which is the only cross-DB join in this design), so every filter in this section is an ordinary SQL join — no HTTP call, no `_resolve_and_fold_mlas`, no 503 failure mode.

### D13 — Endpoints (additive, router ml_ventas_ops.py)
- `GET /sales`: optional `q`, 4 switches, and the D12a product-level facets (`marcas`, `subcategorias`, `pms`; same param shapes and parsing as their `productos_listing.py` counterparts — CSV of ids/strings); per-order additive: city, province, shipping_substatus, coupon_amount, metrics_state ('ok'|'provisional'|'unresolved'|'recalculating'|'pending'), alert_level (error = unresolved/neto null; warning = provisional/recalculating/iva not reconciling/op or goods unknown; ok), **item_category (RESOLVED)**: `productos_erp.categoria` via `ml_order_item_costos.producto_item_id` → `productos_erp.item_id` (frozen cost linkage, already per order line); NULL when no frozen cost → FE fallback icon Package. ML `raw_item.category_id` rejected: opaque MLA category ids, no human mapping stored.
- `GET /sales/kpis`: as rev 1 plus `recalculating_count`, `pending_count`, `worker_alive`; accepts the same D12a product-level facets as `GET /sales` (shared `SalesFilter`), so KPI parity (spec KPI R14) extends to product-filtered combinations.
...
- `POST /orders/{id}/resync` (perm `ml_ops.resincronizar`): HTTP fetch before write block, ingestion writes → triggers → worker. Because triggers ignore no-op writes (D3 rev 5), a resync that re-fetches identical data would enqueue nothing, so the resync write block ALSO calls `order_metrics_enqueue_system([order_id], 'resync')` in the same transaction: every successful resync recomputes the order, whether or not its inputs changed. Endpoint then polls (≤ 2 s, 100 ms step, short read blocks) for the dirty row to disappear and returns the fresh detail; on timeout returns 202-style body with `metrics_state='recalculating'` (spec R24: explicit, not silent). Concurrent resyncs: per-order `pg_try_advisory_xact_lock(order_id)` around the write block → second request gets 409.
...
## Data Flow
    ...
    pricing-worker (systemd)  LISTEN via DATABASE_URL_DIRECT  | safety poll 5s | startup drain
        ...
        | COMPUTE phase (read-only, no row locks): bulk compute_order_metrics for the batch
        |   over batch_timeout or bulk exception: release claims uncharged + suspect, retry each order alone
        | STORE phase, one short txn PER ORDER: FOR UPDATE fence on claim_token ->
        |   owner: upsert ml_order_metrics + ml_venta_deducciones ; DELETE if version unchanged else unclaim ; commit
        |   not owner: rollback (no stale write)
        |   deadline reached mid-store: stop; unstored orders released uncharged, NOT suspect
...
| backend/app/services/ml_sales_query/{filters,search,aggregate}.py | Create | shared scope + aggregation; D12a product-level facets |
| backend/alembic/versions/2026MMDD_ix_ml_order_item_costos_producto_item_id.py | Create | index only, own migration, downgrade drops it (D12a, spec R43) |
...
| Integration PG | claim SKIP LOCKED (two claimers disjoint); lease expiry reclaims; write during recompute ⇒ version-checked delete leaves row; poisoned after 5 attempts; per-order store-transaction isolation (at most one row lock held at a time; design D5 rev 6) | committed-data fixture |
...
| Integration PG | worker loop end-to-end: write+commit ⇒ order goes recalculating ⇒ metrics row stored (asserted as a state transition under a generous bounded wait, never a hard 1 s wall-clock assertion); missed notify recovered by safety poll; startup drain; poll_only mode without direct URL | run runtime in thread with short intervals |
...
**Facet value contract (D12a).** `marcas`: brand NAMES (strings, compared case-insensitively like `productos_listing.py` does with `func.upper`); `subcategorias` and `pms`: integer ids. Parsing is shared by the listing and the KPI: a value of the wrong type (a non-numeric id where an id is expected) or an empty CSV entry is HTTP 422; duplicates are de-duplicated; an id or brand name that exists in no product is accepted and simply matches nothing (never an error, never "no filter").
