...
- **Group (pack) semantics (spec R38)**: identical to the existing promo group semantics — filter selects GROUPS (a pack, or a lone order as its own single-member group); the group matches if ANY member order-item matches; every member is returned. Implemented as an `EXISTS` per filter against the group's member order-items, OR'd across the group, not a per-row AND.
