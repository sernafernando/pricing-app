...
- **Index (spec R43, own earlier PR)**: `ml_order_item_costos.producto_item_id` has NO existing index (verified: only `order_id`, `item_id`, `id` are indexed on that model) and every filter above joins through it. A new index `ix_ml_order_item_costos_producto_item_id` ships as its OWN Alembic migration (`20260922_ix_producto_item_id.py`, single head, downgrade drops it), landing BEFORE the PR that adds these joins (PR9, see tasks.md).
...
| backend/alembic/versions/20260922_ix_producto_item_id.py | Create | index only, own migration, downgrade drops it (D12a, spec R43) |
