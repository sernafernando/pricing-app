...
`INSERT ... SELECT unnest(ids) ON CONFLICT (order_id) DO UPDATE SET version = dirty.version + 1, enqueued_at = now(), reason = EXCLUDED.reason, claimed_at = NULL, claimed_by = NULL` then `PERFORM pg_notify('order_metrics_dirty', '')`. Empty payload on purpose: Postgres folds identical (channel, payload) notifications within one transaction into ONE delivery, so a 10k-row fan-out wakes the worker once. Row triggers for per-order tables; STATEMENT-level triggers with transition tables (`REFERENCING OLD TABLE / NEW TABLE`) for config tables and bulk-prone tables so fan-outs are one set-based insert, not N.
...
1. **Claim** (own txn): `UPDATE ml_order_metrics_dirty d SET claimed_at = now(), claimed_by = :me, attempts = attempts + 1 FROM (SELECT order_id FROM ml_order_metrics_dirty WHERE claimed_at IS NULL OR claimed_at < now() - :lease ORDER BY enqueued_at LIMIT :n FOR UPDATE SKIP LOCKED) c WHERE d.order_id = c.order_id RETURNING d.order_id, d.version`. Commit → locks released immediately, so writers' trigger upserts never wait on the worker. Lease (default 120 s) reclaims rows of a crashed worker; SKIP LOCKED lets a future second worker coexist.
