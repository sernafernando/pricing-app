...
- KPI: orders in the queue are excluded from all sums and reported as `recalculating_count`; orders with no `ml_order_metrics` row yet (`pending`, spec R2) are ALSO excluded and reported separately as `pending_count` (never summed as NULL, never silently dropped); UI shows "N recalculando" chip. Never a stale number as good.
