# ml-ventas-neto Specification

## Purpose

Displayed Neto computation for an ML order: seller's effective net received amount plus SIRTAC add-back, with sobretasa/débitos-créditos/generic Retención handled as before, and the two computation paths (`compute_breakdown`, `compute_neto_by_order_ids`) always in agreement.
