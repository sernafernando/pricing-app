...
class ComisionVersion(Base):
    ...
    __table_args__ = (
        CheckConstraint('fecha_hasta IS NULL OR fecha_hasta >= fecha_desde', name='chk_fechas'),
    )
...
class ComisionBase(Base):
    ...
    __table_args__ = (
        CheckConstraint('grupo_id > 0', name='chk_grupo_positivo'),
    )
...
class ComisionAdicionalCuota(Base):
    ...
    __table_args__ = (
        CheckConstraint('cuotas IN (3, 6, 9, 12)', name='chk_cuotas_validas'),
    )
