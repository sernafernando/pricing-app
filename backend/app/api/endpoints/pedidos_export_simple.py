...
def obtener_pedidos(
    ...
    buscar: Optional[str] = Query(
        None,
        description="Búsqueda universal: cliente, dirección, orden TN, ID pedido, provincia, ciudad, destinatario, observaciones",
    ),
    ...
    query = (
        db.query(SaleOrderHeader, TBCustomer.cust_name.label("nombre_cliente"), TBUser.user_name.label("user_name"))
        .outerjoin(
            TBCustomer,
            and_(SaleOrderHeader.cust_id == TBCustomer.cust_id, SaleOrderHeader.comp_id == TBCustomer.comp_id),
        )
        .outerjoin(TBUser, SaleOrderHeader.user_id == TBUser.user_id)
        .filter(SaleOrderHeader.export_id == 80)
    )
    ...
    if solo_tn:
        ...
        query = query.filter(and_(SaleOrderHeader.ws_internalid.isnot(None), SaleOrderHeader.ws_internalid != ""))
    ...
    if solo_sin_direccion:
        ...
        query = query.filter(
            and_(
                # No tiene override
                or_(
                    SaleOrderHeader.override_shipping_address.is_(None), SaleOrderHeader.override_shipping_address == ""
                ),
                # No tiene TN
                or_(
                ...
                # No tiene ERP
                or_(SaleOrderHeader.soh_deliveryaddress.is_(None), SaleOrderHeader.soh_deliveryaddress == ""),
            )
        )
    ...
    for row in pedidos_db:
        ...
        item_id_efectivo = func.coalesce(SaleOrderDetail.item_id, SaleOrderDetail.sod_item_id_origin).label(
            "item_id_efectivo"
        )

        items_query = (
            db.query(item_id_efectivo, SaleOrderDetail.sod_qty, TBItem.item_desc, TBItem.item_code)
            .outerjoin(
                TBItem,
                and_(
        ...
        items = [
            ...
            if row[0] is not None  # Filtrar solo si item_id existe
        ]
...
def obtener_estadisticas(db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    ...
    base_query = db.query(SaleOrderHeader).filter(
        and_(SaleOrderHeader.export_id == 80, SaleOrderHeader.export_activo == True)
    )
    ...
    total_items = (
        db.query(func.count(SaleOrderDetail.item_id))
        .filter(
    ...
    con_tiendanube = base_query.filter(
        and_(SaleOrderHeader.ws_internalid.isnot(None), SaleOrderHeader.ws_internalid != "")
    ).count()

    con_mercadolibre = base_query.filter(
        and_(SaleOrderHeader.soh_mlid.isnot(None), SaleOrderHeader.soh_mlid != "")
    ).count()
    ...
    sin_direccion = base_query.filter(
        and_(
            # No tiene override
            or_(SaleOrderHeader.override_shipping_address.is_(None), SaleOrderHeader.override_shipping_address == ""),
            # No tiene TN
            or_(
                SaleOrderHeader.tiendanube_shipping_address.is_(None), SaleOrderHeader.tiendanube_shipping_address == ""
            ),
            # No tiene ERP
            or_(SaleOrderHeader.soh_deliveryaddress.is_(None), SaleOrderHeader.soh_deliveryaddress == ""),
        )
    ...
    ultima_sync = (
        db.query(func.max(SaleOrderHeader.soh_lastupdate))
        .filter(and_(SaleOrderHeader.export_id == 80, SaleOrderHeader.export_activo == True))
        .scalar()
    )
...
def obtener_usuarios_disponibles(db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    """
    ...
    usuarios = (
        db.query(SaleOrderHeader.user_id, TBUser.user_name)
        .outerjoin(TBUser, SaleOrderHeader.user_id == TBUser.user_id)
        .filter(
            and_(
        ...
        .distinct()
        .order_by(TBUser.user_name.asc().nullslast())
        .all()
    )
...
def obtener_provincias_disponibles(db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    """
    ...
    provincias = (
        ...
        .distinct()
        .order_by(provincia_efectiva.asc())
        .all()
    )
...
def actualizar_bultos_domicilio(
    soh_id: int,
...
def actualizar_direccion_envio(
    soh_id: int,
...
def eliminar_override_direccion(
    soh_id: int, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)
):
    """
    ...
    return {"mensaje": "Override eliminado, usando datos originales", "soh_id": soh_id}
...
def generar_etiqueta_zpl(
    ...
    items_query = (
        ...
        .outerjoin(TBItem, and_(SaleOrderDetail.item_id == TBItem.item_id, SaleOrderDetail.comp_id == TBItem.comp_id))
        .filter(
            and_(
                SaleOrderDetail.soh_id == pedido.soh_id,
    ...
    return Response(
        ...
        headers={"Content-Disposition": f"attachment; filename=etiqueta_pedido_{soh_id}.txt"},
    )
