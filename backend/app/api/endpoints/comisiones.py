...
async def listar_versiones_comisiones(db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    ...
    for v in versiones:
        ...
        adicionales = [
            ComisionAdicionalResponse(cuotas=ac.cuotas, adicional=float(ac.adicional)) for ac in v.adicionales_cuota
        ]

        resultado.append(
            ComisionVersionResponse(
                id=v.id,
                nombre=v.nombre,
...
async def obtener_version_comision(
    ...
    adicionales = [
        ComisionAdicionalResponse(cuotas=ac.cuotas, adicional=float(ac.adicional)) for ac in version.adicionales_cuota
    ]
    ...
    version = (
        ...
        .filter(
            and_(
                ...
                or_(ComisionVersion.fecha_hasta.is_(None), ComisionVersion.fecha_hasta >= fecha),
...
async def crear_nueva_version_comisiones(
    ...
    version_actual = (
        db.query(ComisionVersion).filter(ComisionVersion.activo == True, ComisionVersion.fecha_hasta.is_(None)).first()
    )
    ...
    for cb_data in data.comisiones_base:
        cb = ComisionBase(version_id=nueva_version.id, grupo_id=cb_data.grupo_id, comision_base=cb_data.comision_base)
        db.add(cb)

    # Crear adicionales
    for ac_data in data.adicionales_cuota:
        ac = ComisionAdicionalCuota(version_id=nueva_version.id, cuotas=ac_data.cuotas, adicional=ac_data.adicional)
        db.add(ac)
...
async def actualizar_version_comisiones(
    ...
    for cb_data in data.comisiones_base:
        cb = ComisionBase(version_id=version_id, grupo_id=cb_data.grupo_id, comision_base=cb_data.comision_base)
        db.add(cb)

    for ac_data in data.adicionales_cuota:
        ac = ComisionAdicionalCuota(version_id=version_id, cuotas=ac_data.cuotas, adicional=ac_data.adicional)
        db.add(ac)
...
async def eliminar_version_comisiones(
    ...
    ultima_version = db.query(ComisionVersion).order_by(ComisionVersion.fecha_creacion.desc()).first()
    ...
    version_anterior = (
        db.query(ComisionVersion)
        .filter(ComisionVersion.id != version_id, ComisionVersion.fecha_hasta.isnot(None))
        .order_by(ComisionVersion.fecha_desde.desc())
        .first()
    )
    ...
    return {
        ...
        "version_reactivada": {"id": version_anterior.id, "nombre": version_anterior.nombre}
        if version_anterior
        else None,
    }
