...
async def obtener_pedidos_por_export(
    ...
    """
    # Query principal
    query = db.query(SaleOrderHeader).filter(SaleOrderHeader.export_id == export_id)
...
async def obtener_estadisticas_export(
    ...
    # Total de pedidos
    total_pedidos = (
        db.query(func.count(SaleOrderHeader.soh_id)).filter(SaleOrderHeader.export_id == export_id).scalar() or 0
    )

    # Total de bultos
    total_bultos = (
        db.query(func.sum(SaleOrderHeader.soh_packagesqty)).filter(SaleOrderHeader.export_id == export_id).scalar() or 0
    )

    # Pendientes de etiqueta (sin codigo_envio_interno)
    pendientes_etiqueta = (
        db.query(func.count(SaleOrderHeader.soh_id))
        .filter(and_(SaleOrderHeader.export_id == export_id, SaleOrderHeader.codigo_envio_interno.is_(None)))
        .scalar()
        or 0
    )
    ...
    con_ml = (
        db.query(func.count(SaleOrderHeader.soh_id))
        .filter(and_(SaleOrderHeader.export_id == export_id, SaleOrderHeader.mlo_id.isnot(None)))
        .scalar()
        or 0
    )
    ...
    con_tiendanube = (
        db.query(func.count(SaleOrderHeader.soh_id))
        .filter(and_(SaleOrderHeader.export_id == export_id, SaleOrderHeader.ws_internalid.isnot(None)))
        .scalar()
        or 0
    )
    ...
    ultima_actualizacion = (
        db.query(func.max(SaleOrderHeader.soh_lastupdate)).filter(SaleOrderHeader.export_id == export_id).scalar()
    )
...
async def asignar_codigo_envio(
    ...
    existe = (
        db.query(SaleOrderHeader)
        .filter(and_(SaleOrderHeader.codigo_envio_interno == codigo, SaleOrderHeader.soh_id != soh_id))
        .first()
    )
...
async def sincronizar_export_80(
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=300.0) as client:
            response = await client.post("http://localhost:8002/api/gbp-parser", json={"intExpgr_id": 87})
...
async def procesar_pedidos_export_80_async(data: List[Dict[str, Any]], db: Session, force_full: bool = False):
    ...
    try:
        ...
        pedidos_filtrados = db.query(SaleOrderHeader.soh_id).filter(SaleOrderHeader.soh_id.in_(soh_ids_actuales)).all()
        ...
        pedidos_con_items = (
            db.query(SaleOrderDetail.soh_id).filter(SaleOrderDetail.soh_id.in_(soh_ids_filtrados)).distinct().all()
        )
        ...
        pedidos_solo_servicio = (
            db.query(SaleOrderDetail.soh_id)
            .filter(SaleOrderDetail.soh_id.in_(soh_ids_con_items))
            .group_by(SaleOrderDetail.soh_id)
            .having(
                func.count(SaleOrderDetail.item_id)
                == func.sum(case((SaleOrderDetail.item_id.in_([2953, 2954]), 1), else_=0))
            )
            .all()
        )
        ...
        pedidos_archivados_count = (
            ...
            .update({"export_activo": False}, synchronize_session=False)
        )
        ...
        pedidos_sin_codigo = (
            ...
            .filter(
                and_(
                    ...
                    or_(SaleOrderHeader.codigo_envio_interno.is_(None), SaleOrderHeader.codigo_envio_interno == ""),
                )
            )
            .all()
        )
...
async def estadisticas_sincronizacion(db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    archivados = (
        db.query(func.count(SaleOrderHeader.soh_id))
        .filter(and_(SaleOrderHeader.export_id == 80, SaleOrderHeader.export_activo == False))
        .scalar()
        or 0
    )
