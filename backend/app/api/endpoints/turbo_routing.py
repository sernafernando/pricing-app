...
# ============================================================================
# ENDPOINTS: BANLIST DE ENVÍOS TURBO
# ============================================================================
...
async def listar_envios_baneados(
    ...
    banlist = db.query(EnvioTurboBanlist).order_by(
        EnvioTurboBanlist.baneado_at.desc()
...
async def banear_envio_turbo(
    ...
    existente = db.query(EnvioTurboBanlist).filter(
        EnvioTurboBanlist.mlshippingid == request.mlshippingid
    ).first()
    
    if existente:
        raise HTTPException(
            status_code=400,
            detail=f"El envío {request.mlshippingid} ya está en la banlist"
        )
    ...
    logger.info(f"Envío {request.mlshippingid} agregado a banlist por usuario {current_user.get('nombre')}: {request.motivo}")
    
    return {
        "success": True,
        "message": f"Envío {request.mlshippingid} agregado a la banlist",
        "id": nuevo_ban.id
    }
...
async def desbanear_envio_turbo(
    ...
    ban = db.query(EnvioTurboBanlist).filter(
        EnvioTurboBanlist.id == banlist_id
    ).first()
