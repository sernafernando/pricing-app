...
async def obtener_pedidos(
    ...
    for p in pedidos:
        ...
        items_query = (
            db.query(PedidoExport.item_id, PedidoExport.cantidad).filter(PedidoExport.id_pedido == p.id_pedido).all()
        )
...
async def obtener_estadisticas(
    ...
    total_pedidos = (
        db.query(func.count(func.distinct(PedidoExport.id_pedido))).filter(PedidoExport.activo == True).scalar() or 0
    )

    total_items = db.query(func.count(PedidoExport.item_id)).filter(PedidoExport.activo == True).scalar() or 0

    con_tiendanube = (
        db.query(func.count(func.distinct(PedidoExport.id_pedido)))
        .filter(and_(PedidoExport.activo == True, PedidoExport.order_id_tn.isnot(None)))
        .scalar()
        or 0
    )
    ...
    sin_direccion = (
        db.query(func.count(func.distinct(PedidoExport.id_pedido)))
        .filter(and_(PedidoExport.activo == True, PedidoExport.direccion_envio.is_(None)))
        .scalar()
        or 0
    )

    ultima_sync = db.query(func.max(PedidoExport.fecha_sync)).filter(PedidoExport.activo == True).scalar()
...
async def sincronizar_pedidos(
    ...
    try:
        ...
        # Archivar viejos
        archivados = (
            db.query(PedidoExport)
            .filter(and_(PedidoExport.activo == True, PedidoExport.id_pedido.notin_(pedidos_actuales)))
        ...
        for record in registros_unicos.values():
            ...
            pedido = (
                db.query(PedidoExport)
                .filter(and_(PedidoExport.id_pedido == id_pedido, PedidoExport.item_id == item_id))
                .first()
            )
...
async def enriquecer_tiendanube(
    ...
    return {
        ...
        "pedidos_tn": db.query(func.count(func.distinct(PedidoExport.id_pedido)))
        .filter(and_(PedidoExport.activo == True, PedidoExport.order_id_tn.isnot(None)))
        .scalar()
        or 0,
    }
