...
async def get_items_sin_mla(
    ...
    query = db.query(ProductoERP).filter(
        and_(ProductoERP.activo == True, ProductoERP.item_id.notin_(items_baneados_subq))
    )
    ...
    if buscar:
        ...
        query = query.filter(or_(ProductoERP.codigo.ilike(search_term), ProductoERP.descripcion.ilike(search_term)))
...
async def get_items_baneados(db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    ...
    baneados = (
        db.query(ItemSinMLABanlist, ProductoERP, Usuario)
        .join(ProductoERP, ProductoERP.item_id == ItemSinMLABanlist.item_id)
        .join(Usuario, Usuario.id == ItemSinMLABanlist.usuario_id)
        .order_by(ItemSinMLABanlist.fecha_creacion.desc())
        .all()
    )
...
async def banear_item(
    ...
    existente = db.query(ItemSinMLABanlist).filter(ItemSinMLABanlist.item_id == request.item_id).first()

    if existente:
        raise HTTPException(status_code=400, detail="El item ya está en la banlist")

    # Crear entrada en banlist
    nuevo_ban = ItemSinMLABanlist(item_id=request.item_id, motivo=request.motivo, usuario_id=current_user.id)

    db.add(nuevo_ban)
    db.commit()
    db.refresh(nuevo_ban)

    return {"success": True, "message": f"Item {request.item_id} agregado a la banlist", "banlist_id": nuevo_ban.id}
...
async def desbanear_item(
    ...
    ban_entry = db.query(ItemSinMLABanlist).filter(ItemSinMLABanlist.id == request.banlist_id).first()
    ...
    return {"success": True, "message": f"Item {item_id} removido de la banlist"}
...
async def get_comparacion_listas(
    ...
    query = (
        db.query(MLPublicationSnapshot, MercadoLibreItemPublicado, ProductoERP)
        .join(
            latest_snapshots,
            and_(
        ...
        .join(MercadoLibreItemPublicado, MercadoLibreItemPublicado.mlp_publicationID == MLPublicationSnapshot.mla_id)
        .join(ProductoERP, ProductoERP.item_id == MercadoLibreItemPublicado.item_id)
        .filter(
...
async def get_comparacion_baneados(db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    ...
    baneados = (
        db.query(ComparacionListasBanlist, Usuario)
        .join(Usuario, Usuario.id == ComparacionListasBanlist.usuario_id)
        .order_by(ComparacionListasBanlist.fecha_creacion.desc())
        .all()
    )
    ...
    for banlist_entry, usuario in baneados:
        ...
        publicacion = (
            db.query(MercadoLibreItemPublicado, ProductoERP)
            .join(ProductoERP, ProductoERP.item_id == MercadoLibreItemPublicado.item_id)
            .filter(MercadoLibreItemPublicado.mlp_publicationID == banlist_entry.mla_id)
            .first()
        )
...
async def banear_comparacion(
    ...
    # Verificar que no esté ya baneado
    existente = db.query(ComparacionListasBanlist).filter(ComparacionListasBanlist.mla_id == request.mla_id).first()

    if existente:
        raise HTTPException(status_code=400, detail="La publicación ya está en la banlist de comparación")

    nuevo_ban = ComparacionListasBanlist(mla_id=request.mla_id, motivo=request.motivo, usuario_id=current_user.id)
...
async def desbanear_comparacion(
    ...
    ban_entry = db.query(ComparacionListasBanlist).filter(ComparacionListasBanlist.id == request.banlist_id).first()
    ...
    return {"success": True, "message": f"Publicación {mla_id} removida de la banlist de comparación"}
