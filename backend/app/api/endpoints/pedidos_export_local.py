...
async def obtener_pedidos_local(
    ...
    for pedido, nombre_cliente, user_name, tn_orderid, tn_json in resultados:
        ...
        for item_id, cantidad, item_desc, item_code in items_query.all():
            items.append(
                ItemPedidoDetalle(item_id=item_id, cantidad=cantidad, item_desc=item_desc, item_code=item_code)
            )
            ...
            try:
                ...
                if shipping_addr and shipping_addr.get("address"):
                    ...
                    tn_data = {
                        ...
                        "tiendanube_number": str(tn_parsed.get("number", ""))
                        if tn_parsed.get("number")
                        else pedido.tiendanube_number,
...
async def obtener_estadisticas_local(
    ...
    # Total de items
    query = (
        db.query(func.sum(SaleOrderDetail.sod_qty))
        .join(
            SaleOrderHeader,
            and_(SaleOrderDetail.soh_id == SaleOrderHeader.soh_id, SaleOrderDetail.bra_id == SaleOrderHeader.bra_id),
        )
        .filter(
            or_(
                func.coalesce(SaleOrderDetail.item_id, SaleOrderDetail.sod_item_id_origin).is_(None),
    ...
    query = (
        ...
        .filter(TiendaNubeOrder.tno_orderid.isnot(None))
    )
    if base_filter:
        query = query.filter(and_(*base_filter))
