"""
Script para sincronizar tb_rma_supplier_cn_pending desde el ERP (tbRMA_SupplierCreditNotePending)

Modos de uso:
    # Full - toda la tabla del ERP (paginado por cursor)
    python -m app.scripts.sync_rma_supplier_cn_pending --full

    # Incremental automático (desde último rmanc_id en DB)
    python -m app.scripts.sync_rma_supplier_cn_pending --incremental

    # Incremental desde un ID específico
    python -m app.scripts.sync_rma_supplier_cn_pending --incremental --from-id 500

    # Por RMA header específico
    python -m app.scripts.sync_rma_supplier_cn_pending --rmah-id 406

    # Por RMA detail específico
    python -m app.scripts.sync_rma_supplier_cn_pending --rmad-id 83681
"""
...
def normalize_row(row: dict) -> dict | None:
    ...
    if not row.get("comp_id") or not row.get("rmanc_id"):
        return None
    cleaned = {}
...
def upsert_batch(db: Session, rows: list[dict]) -> None:
    """Upsert de un batch de filas en tb_rma_supplier_cn_pending."""
    stmt = insert(TbRMASupplierCNPending)
    stmt = stmt.on_conflict_do_update(
        index_elements=["comp_id", "rmanc_id"],
...
def get_max_rmanc_id(db: Session) -> int:
    """Obtiene el máximo rmanc_id en la DB local."""
    result = db.query(func.max(TbRMASupplierCNPending.rmanc_id)).scalar()
    return result or 0


def sync_full(db: Session) -> None:
    """Sincronización completa usando cursor por rmanc_id (rmanc_id > X)"""
    print("\n🔄 Sincronización COMPLETA de tb_rma_supplier_cn_pending")
    print("=" * 60)
    ...
    while True:
        print(f"📦 Lote #{batch_num} (rmanc_id > {cursor})...")

        params = {
            "strScriptLabel": "scriptRMASupplierCreditNotePending",
            "rmancID": cursor,
        }
        ...
        try:
            ...
            max_id_in_batch = max(row["rmanc_id"] for row in normalized_data)
...
def sync_incremental(db: Session, from_id: int | None = None) -> None:
    """Sincronización incremental desde el último rmanc_id en la DB (o desde from_id)"""
    last_rmanc_id = from_id if from_id is not None else get_max_rmanc_id(db)

    print(f"\n🔄 Sincronización INCREMENTAL de tb_rma_supplier_cn_pending (desde rmanc_id > {last_rmanc_id})")
    print("=" * 60)

    params = {
        "strScriptLabel": "scriptRMASupplierCreditNotePending",
        "rmancID": last_rmanc_id,
    }

    print(f"📡 Consultando ERP (rmanc_id > {last_rmanc_id})...")
    data = asyncio.run(fetch_from_erp(params))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Sincronizar tb_rma_supplier_cn_pending")

    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument("--full", action="store_true", help="Sincronización completa (toda la tabla del ERP)")
    mode.add_argument("--incremental", action="store_true", help="Incremental desde último rmanc_id en DB")
    mode.add_argument("--rmah-id", type=int, help="Sincronizar CN pendientes de un RMA header específico")
    mode.add_argument("--rmad-id", type=int, help="Sincronizar CN pendientes de un RMA detail específico")

    parser.add_argument(
        "--from-id",
        type=int,
        default=None,
        help="Forzar rmanc_id inicial para incremental (override auto-detect)",
    )
