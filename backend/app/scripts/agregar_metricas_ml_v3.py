"""
Script para agregar métricas de ventas ML - Versión 3
Ejecuta el query de scriptDashboard directamente en SQL Server
Procesa los datos y los inserta en ml_ventas_metricas

Ejecutar:
    python app/scripts/agregar_metricas_ml_v3.py --from-date 2025-01-01 --to-date 2025-11-14
"""
...
def get_sql_server_connection():
    ...
    return pymssql.connect(
        server=SQL_SERVER_HOST, database=SQL_SERVER_DB, user=SQL_SERVER_USER, password=SQL_SERVER_PASSWORD, timeout=120
    )
...
def process_and_insert(db: Session, df: pd.DataFrame, min_free: float = 33000):
    ...
    # Esto es equivalente a lo que hace st_app
    ...
    for idx, row in df.iterrows():
        try:
            ...
            existente = db.query(MLVentaMetrica).filter(MLVentaMetrica.id_operacion == row["ID_de_Operación"]).first()
            ...
            if existente:
                ...
                existente.pack_id = (
                    ...
                    if pd.notna(row.get("ML_pack_id")) and str(row.get("ML_pack_id")).replace(".", "").isdigit()
                    else None
                )
...
def main():
    ...
    # Agregar 1 día para incluir todo el día to_date
    ...
    df = fetch_data_from_sql_server(from_date, to_date_inclusive)
