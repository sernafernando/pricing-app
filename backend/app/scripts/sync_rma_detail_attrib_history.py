"""
Script para sincronizar tb_rma_detail_attrib_history desde el ERP (tbRMA_Detail_AttributesHistory)

Modos de uso:
    # Full - toda la tabla del ERP (paginado por cursor)
    python -m app.scripts.sync_rma_detail_attrib_history --full

    # Incremental automático (desde último rmadh_id en DB)
    python -m app.scripts.sync_rma_detail_attrib_history --incremental

    # Incremental desde un ID específico
    python -m app.scripts.sync_rma_detail_attrib_history --incremental --from-id 500

    # Por RMA header específico
    python -m app.scripts.sync_rma_detail_attrib_history --rmah-id 320

    # Por RMA detail específico
    python -m app.scripts.sync_rma_detail_attrib_history --rmad-id 83579
"""
...
def normalize_row(row: dict) -> dict | None:
    ...
    if not row.get("comp_id") or not row.get("rmadh_id"):
        return None
    cleaned = {}
...
def upsert_batch(db: Session, rows: list[dict]) -> None:
    """Upsert de un batch de filas en tb_rma_detail_attrib_history."""
    stmt = insert(TbRMADetailAttribHistory)
    stmt = stmt.on_conflict_do_update(
        index_elements=["comp_id", "rmadh_id"],
...
def get_max_rmadh_id(db: Session) -> int:
    """Obtiene el máximo rmadh_id en la DB local."""
    result = db.query(func.max(TbRMADetailAttribHistory.rmadh_id)).scalar()
    return result or 0


def sync_full(db: Session) -> None:
    """Sincronización completa usando cursor por rmadh_id (rmadh_id > X)"""
    print("\n🔄 Sincronización COMPLETA de tb_rma_detail_attrib_history")
    print("=" * 60)
    ...
    while True:
        print(f"📦 Lote #{batch_num} (rmadh_id > {cursor})...")

        params = {
            "strScriptLabel": "scriptRMADetailAttribHistory",
            "rmadhID": cursor,
        }
        ...
        try:
            ...
            max_id_in_batch = max(row["rmadh_id"] for row in normalized_data)
...
def sync_incremental(db: Session, from_id: int | None = None) -> None:
    """Sincronización incremental desde el último rmadh_id en la DB (o desde from_id)"""
    last_rmadh_id = from_id if from_id is not None else get_max_rmadh_id(db)

    print(f"\n🔄 Sincronización INCREMENTAL de tb_rma_detail_attrib_history (desde rmadh_id > {last_rmadh_id})")
    print("=" * 60)

    params = {
        "strScriptLabel": "scriptRMADetailAttribHistory",
        "rmadhID": last_rmadh_id,
    }

    print(f"📡 Consultando ERP (rmadh_id > {last_rmadh_id})...")
    data = asyncio.run(fetch_from_erp(params))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Sincronizar tb_rma_detail_attrib_history")

    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument("--full", action="store_true", help="Sincronización completa (toda la tabla del ERP)")
    mode.add_argument("--incremental", action="store_true", help="Incremental desde último rmadh_id en DB")
    mode.add_argument("--rmah-id", type=int, help="Sincronizar historial de un RMA header específico")
    mode.add_argument("--rmad-id", type=int, help="Sincronizar historial de un RMA detail específico")

    parser.add_argument(
        "--from-id",
        type=int,
        default=None,
        help="Forzar rmadh_id inicial para incremental (override auto-detect)",
    )
