"""
Script para sincronizar tb_rma_detail desde el ERP (tbRMA_Detail)

Modos de uso:
    # Full - toda la tabla del ERP (paginado por cursor)
    python -m app.scripts.sync_rma_detail --full

    # Incremental automático (desde último rmad_id en DB)
    python -m app.scripts.sync_rma_detail --incremental

    # Incremental desde un ID específico
    python -m app.scripts.sync_rma_detail --incremental --from-id 90000

    # Por RMA header específico
    python -m app.scripts.sync_rma_detail --rmah-id 8791

    # Por rango de fecha de recepción
    python -m app.scripts.sync_rma_detail --from-date 2026-01-01 --to-date 2026-02-25
"""
...
# Campos válidos de la tabla (para filtrar basura del ERP)
VALID_FIELDS = {
    "comp_id",
    "rmah_id",
    "rmad_id",
    ...
    "rmad_isNewItem",
    "supp_id",
    "impData_Custom_local",
    "impData_Number_local",
    "rmad_IncludeInPotentialStock",
    "rmas_id",
    "rmap_id",
    "item_id4Replacement",
    "rmamt_id",
    ...
    "case_id",
    "rmad_relatedID",
    "it_transaction_Origin",
    "rmad_guid",
    "rmad_insertWarrantyDetail",
    "rmad_insertWarrantyCertificates",
    "bra_id_original",
    "rmad_isAvailable4DeliverySheet",
    "sds_id4Picking",
    "dsd_id4Picking",
    "sds_id4Delivery",
    "dsd_id4Delivery",
    "dl_id4Picking",
    "dl_id4delivery",
    "rmad_deliveryDate4Picking",
    "rmad_deliveryDate4Delivery",
    "rmad_Date_estimatedFinalization",
    "rmad_handOverDate",
    "rmad_handOver_user_id",
}

# Campos PK (no se actualizan en upsert)
PK_FIELDS = {"comp_id", "rmad_id", "bra_id"}

# Campos que se actualizan en upsert (todos menos PK)
...
def normalize_row(row: dict) -> dict | None:
    ...
    if not row.get("comp_id") or not row.get("rmad_id") or not row.get("bra_id"):
        return None
    cleaned = {}
...
def upsert_batch(db: Session, rows: list[dict]) -> None:
    """Upsert de un batch de filas en tb_rma_detail."""
    stmt = insert(TbRMADetail)
    stmt = stmt.on_conflict_do_update(
        index_elements=["comp_id", "rmad_id", "bra_id"],
...
def get_max_rmad_id(db: Session) -> int:
    """Obtiene el máximo rmad_id en la DB local."""
    result = db.query(func.max(TbRMADetail.rmad_id)).scalar()
    return result or 0


def sync_full(db: Session) -> None:
    """Sincronización completa usando cursor por rmad_id (rmad_id > X)"""
    print("\n🔄 Sincronización COMPLETA de tb_rma_detail")
    print("=" * 60)
    ...
    while True:
        print(f"📦 Lote #{batch_num} (rmad_id > {cursor})...")

        params = {
            "strScriptLabel": "scriptRMADetail",
            "rmadID": cursor,
        }
        ...
        try:
            ...
            # Avanzar cursor al max rmad_id de este lote
            max_id_in_batch = max(row["rmad_id"] for row in normalized_data)
...
def sync_incremental(db: Session, from_id: int | None = None) -> None:
    """Sincronización incremental desde el último rmad_id en la DB (o desde from_id)"""
    last_rmad_id = from_id if from_id is not None else get_max_rmad_id(db)

    print(f"\n🔄 Sincronización INCREMENTAL de tb_rma_detail (desde rmad_id > {last_rmad_id})")
    print("=" * 60)
    ...
    print(f"📡 Consultando ERP (rmad_id > {last_rmad_id})...")
    data = asyncio.run(fetch_from_erp(params))
...
def sync_by_rmah(db: Session, rmah_id: int) -> None:
    """Sincronización por RMA header específico"""
    print(f"\n🔄 Sincronización por RMA header (rmah_id={rmah_id})")
    print("=" * 60)
    ...
    print(f"📡 Consultando ERP (rmah_id={rmah_id})...")
    data = asyncio.run(fetch_from_erp(params))

    if not data:
        print(f"⚠️  No se encontraron detalles para rmah_id={rmah_id}")
        return
...
def sync_by_date_range(db: Session, from_date: str, to_date: str) -> None:
    """Sincronización por rango de fecha de recepción"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Sincronizar tb_rma_detail")

    # Modos de ejecución
    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument("--full", action="store_true", help="Sincronización completa (toda la tabla del ERP)")
    mode.add_argument("--incremental", action="store_true", help="Incremental desde último rmad_id en DB")
    mode.add_argument("--rmah-id", type=int, help="Sincronizar detalles de un RMA header específico")
    mode.add_argument("--date-range", action="store_true", help="Sincronizar por rango de fechas")

    # Opciones adicionales
    parser.add_argument(
        "--from-id", type=int, default=None, help="Forzar rmad_id inicial para incremental (override auto-detect)"
    )
