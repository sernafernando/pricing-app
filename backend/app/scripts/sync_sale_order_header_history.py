...
def sync_sale_order_header_history(db: Session, data: list):
    ...
    for record in data:
        try:
            ...
            existente = (
                db.query(SaleOrderHeaderHistory)
                .filter(
                    and_(
            ...
            errores += 1
            print(
                f"  ⚠️  Error en registro (comp_id={record.get('comp_id')}, bra_id={record.get('bra_id')}, soh_id={record.get('soh_id')}, sohh_id={record.get('sohh_id')}): {str(e)}"
            )
