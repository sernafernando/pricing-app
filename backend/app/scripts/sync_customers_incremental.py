...
def fetch_customers_from_erp(
    cust_id: int = None,
    from_cust_id: int = None,
    to_cust_id: int = None
):
    """
...
def sync_customers(
    ...
    try:
        ...
        if cust_id:
            ...
            if not from_cust_id:
                last_record = db_local.query(TBCustomer).order_by(
            ...
            registros_erp = fetch_customers_from_erp(
                from_cust_id=from_id,
                to_cust_id=to_id
            )
        ...
        existing = db_local.query(TBCustomer.cust_id).filter(
        ...
        for record in registros_erp:
            ...
            if cust_id_val in ids_existentes:
                ...
                db_local.query(TBCustomer).filter(
                    TBCustomer.comp_id == datos['comp_id'],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--from-id',
        type=int,
        help='ID de cliente desde'
    )
    parser.add_argument(
        '--to-id',
        type=int,
        help='ID de cliente hasta'
    )
    parser.add_argument(
        '--cust-id',
        type=int,
        help='ID de cliente específico'
    )
    parser.add_argument(
        '--batch-size',
        type=int,
    ...
    sync_customers(
        from_cust_id=args.from_id,
        to_cust_id=args.to_id,
