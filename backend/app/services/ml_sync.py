    ...
    try:
        ...
        for row in data:
            try:
                ...
                producto_existe = db.query(ProductoERP).filter(ProductoERP.item_id == int(item_id)).first()
...
def obtener_publicaciones_por_item(db: Session, item_id: int) -> List[PublicacionML]:
    ...
    return db.query(PublicacionML).filter(PublicacionML.item_id == item_id).all()


def obtener_publicacion_por_mla(db: Session, mla: str) -> PublicacionML:
    """Obtiene una publicación por su MLA"""
    return db.query(PublicacionML).filter(PublicacionML.mla == mla).first()
