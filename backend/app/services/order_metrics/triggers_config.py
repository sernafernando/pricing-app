"""Config/statement-level enqueue triggers (ventas-ml-rediseno PR5, design D3
statement-level scope, D11). Postgres-only DDL, same immutability contract
and `create_*`/`drop_*` invocation pattern as `triggers.py` (PR4): applied
EXPLICITLY by the dedicated `pg_order_metrics_config_triggers_engine` test
fixture (`tests/conftest.py`) and, as a FROZEN point-in-time copy inlined at
authoring time, by the PR5 Alembic migration
(`alembic/versions/20260923_ml_order_metrics_triggers_config.py`). Never by
a global SQLAlchemy metadata event -- see `triggers.py`'s module docstring
for the exact reasoning (shared `ml_order_metrics_dirty` table, pre-existing
PR1/PR3 fixtures).

Migration immutability contract (unchanged from `triggers.py`): this module
is the LIVE definition. Once the PR5 migration has inlined a frozen copy of
these statements, that copy is never re-synced when this module changes. A
later PR that needs to add or change THIS trigger DDL adds a NEW list (or a
NEW module) and a NEW migration that inlines its own frozen copy.

`etiquetas_envio` gets ROW-level triggers (`UPDATE OF <cols>` + a row-level
`WHEN` guard is valid there, same shape as PR4's per-order tables). The five
config tables (`ml_venta_varios_pct`, `logistica_costo_cordon`,
`cp_cordones`, `configuracion`, `transportes`) get STATEMENT-level triggers
with `REFERENCING OLD TABLE / NEW TABLE` -- Postgres forbids combining
transition tables with a column list, and a statement-level `WHEN` cannot
read row values (design D3 rev 7), so every column/key filter and the
no-op-write guard live INSIDE the trigger function, comparing `old_table`
against `new_table`.

Every affected-order resolution below joins through `etiquetas_envio` (keyed
by `shipping_id`, the same string-cast join
`_resolve_flex_cost_by_shipping_id` (breakdown_service.py) uses) because
that is the ONLY table the Flex cost formula actually reads these config
rows THROUGH -- an order with no label is never affected by a config-only
change, no matter what its other data looks like.
"""
...
# five config tables' functions JOIN it.
...
def create_config_triggers(bind: Connection) -> None:
    """Applies every config/statement-level enqueue trigger, in dependency
    order. Postgres-only -- requires `order_metrics_enqueue` (triggers.py)
    to already exist on the bound database."""
    for statement in _CREATE_STATEMENTS:
        bind.exec_driver_sql(statement)


def drop_config_triggers(bind: Connection) -> None:
    """Reverse of `create_config_triggers`."""
