...
def generar_zonas_kmeans(
    envios_coords: List[Tuple[float, float, int]],
    cantidad_zonas: int
) -> List[Dict[str, Any]]:
    """
    ...
    if len(envios_coords) < cantidad_zonas:
        logger.warning(
            f"Solo hay {len(envios_coords)} envíos geocodificados, "
            f"menos que {cantidad_zonas} zonas solicitadas"
        )
...
def generar_buffer_circular(coords: np.ndarray, radio_km: float = 1.0) -> Dict[str, Any]:
    ...
    circulo_coords = [
        (
            centroid_lng + radio_grados * np.cos(angulo),
            centroid_lat + radio_grados * np.sin(angulo)
        )
...
def validar_envios_geocodificados(
    total_envios: int,
    envios_geocodificados: int,
    porcentaje_minimo: float = 0.7
) -> Tuple[bool, str]:
    """
