                ...
                try:
                    ...
                    if not producto_existente:
                        nuevo_producto = ProductoERP(
                            ...
                            iva=iva,
                            stock=stock,
                            estado=producto_data.get('Estado'),
                            mla=producto_data.get('MLA'),
                            proveedor=producto_data.get('Proveedor', ''),
                            moneda=producto_data.get('Moneda'),
                            precio_publicado=precio_publicado,
                            envio=envio,
                            item_liquidation=producto_data.get('item_liquidation'),
                            hash_datos=hash_nuevo
                        )
                        db.add(nuevo_producto)
                        
                        nuevo_pricing = ProductoPricing(
                            item_id=item_id,
                            precio_lista_ml=None
                        )
                        db.add(nuevo_pricing)
                        ...
                        producto_existente.estado = producto_data.get('Estado')
                        producto_existente.mla = producto_data.get('MLA')
                        producto_existente.moneda = producto_data.get('Moneda')
                        producto_existente.precio_publicado = precio_publicado
