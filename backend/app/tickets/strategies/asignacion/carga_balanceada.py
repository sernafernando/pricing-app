...
class CargaBalanceadaStrategy(AsignacionStrategy):
    ...
    def asignar(self, ticket: Ticket, sector: Sector) -> Optional[Usuario]:
        ...
        carga = (
            self.db.query(AsignacionTicket.asignado_a_id, func.count(AsignacionTicket.id).label("cantidad"))
            .join(Ticket)
        ...
        usuario_menor_carga = min(usuarios_disponibles, key=lambda u: carga_dict.get(u.id, 0))
