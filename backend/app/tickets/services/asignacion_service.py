...
class AsignacionService:
    ...
    def asignar_manual(
        self, ticket: Ticket, usuario_asignado: Usuario, asignado_por: Usuario, motivo: Optional[str] = None
    ) -> AsignacionTicket:
        """
    ...
    def reasignar(
        self, ticket: Ticket, nuevo_usuario: Usuario, usuario_que_reasigna: Usuario, motivo: Optional[str] = None
    ) -> AsignacionTicket:
        """
        ...
        logger.info(
            f"Ticket #{ticket.id} reasignado de usuario #{asignacion_actual.asignado_a_id if asignacion_actual else 'N/A'} a #{nuevo_usuario.id}"
        )
    ...
    def escalar(
        ...
        # Disparar evento
        EventBus.publish("ticket.escalado", ticket=ticket, escalado_por=usuario_escalador, nivel=nivel_escalamiento)
    ...
    def _asignar_ticket(
        ...
        # Disparar evento
        EventBus.publish(
            "ticket.assigned", ticket=ticket, usuario_asignado=usuario_asignado, asignado_por=asignado_por, tipo=tipo
        )
