...
class WorkflowService:
    ...
    def can_transition(
        ...
        if not transicion:
            return False, f"No existe una transición permitida de '{ticket.estado.nombre}' a '{self._get_estado(nuevo_estado_id).nombre}'"
    ...
    def transition(
        ...
        self._crear_historial(
            ...
            descripcion=f"Estado cambiado de '{estado_anterior.nombre}' a '{nuevo_estado.nombre}'" + (f": {comentario}" if comentario else ""),
        ...
        logger.info(f"Ticket #{ticket.id} transicionado de '{estado_anterior.nombre}' a '{nuevo_estado.nombre}' por usuario #{usuario.id}")
    ...
    def _get_transicion(self, estado_origen_id: int, estado_destino_id: int) -> Optional[TransicionEstado]:
        ...
        return self.db.query(TransicionEstado).filter(
