...
class TestCrearNCEndpoint:
    ...
    def test_crear_nc_sin_motivo_422(
        self, client, auth_headers, empresa, proveedor, con_todos_los_permisos
    ):
    ...
    def test_crear_nc_monto_negativo_422(
        self, client, auth_headers, empresa, proveedor, con_todos_los_permisos
    ):
        payload = _nc_payload(empresa.id, proveedor.id)
        payload["monto"] = "-100"
...
class TestListarYDetalle:
    def test_listar_paginado(
        self, client, auth_headers, nc_borrador, con_todos_los_permisos
    ):
    ...
    def test_listar_con_filtro_proveedor(
        self, client, auth_headers, nc_borrador, proveedor, con_todos_los_permisos
    ):
        r = client.get(
            f"{BASE}/ncs-locales?proveedor_id={proveedor.id}", headers=auth_headers
        )
        assert r.status_code == 200
        data = r.json()
        assert all(it["proveedor_id"] == proveedor.id for it in data["items"])

    def test_obtener_detalle(
        self, client, auth_headers, nc_borrador, con_todos_los_permisos
    ):
...
class TestTransiciones:
    def test_enviar_a_aprobacion(
        self, client, auth_headers, nc_borrador, con_todos_los_permisos
    ):
        r = client.post(
            f"{BASE}/ncs-locales/{nc_borrador.id}/enviar-aprobacion", headers=auth_headers
        )
        assert r.status_code == 200
        assert r.json()["estado"] == "pendiente_aprobacion"

    def test_aprobar_sin_permiso_403(
        self, client, auth_headers, nc_borrador, sin_permiso_aprobar_nc
    ):
    ...
    def test_aprobar_con_permiso_200(
        self, client, auth_headers, nc_borrador, con_todos_los_permisos
    ):
        client.post(
            f"{BASE}/ncs-locales/{nc_borrador.id}/enviar-aprobacion", headers=auth_headers
    ...
    def test_rechazar_sin_motivo_400(
        self, client, auth_headers, nc_borrador, con_todos_los_permisos
    ):
        client.post(
            f"{BASE}/ncs-locales/{nc_borrador.id}/enviar-aprobacion", headers=auth_headers
    ...
    def test_cancelar_desde_borrador(
        self, client, auth_headers, nc_borrador, con_todos_los_permisos
    ):
        r = client.post(
    ...
    def test_cancelar_aprobada_sin_motivo_400(
        self, client, auth_headers, nc_borrador, con_todos_los_permisos
    ):
        client.post(
            f"{BASE}/ncs-locales/{nc_borrador.id}/enviar-aprobacion", headers=auth_headers
...
class TestFlujoCompleto:
    def test_crear_enviar_aprobar_imputar_aplicada(
        ...
        r = client.post(
            f"{BASE}/ncs-locales/{nc_id}/enviar-aprobacion", headers=auth_headers
        )
        assert r.status_code == 200
...
class TestEventos:
    def test_listar_eventos_incluye_creacion(
        self, client, auth_headers, nc_borrador, con_todos_los_permisos
    ):
        r = client.get(
            f"{BASE}/ncs-locales/{nc_borrador.id}/eventos", headers=auth_headers
        )
        assert r.status_code == 200
        eventos = r.json()
...
class TestAdjuntosNC:
    def test_subir_adjunto_nc_ok(
        self, client, auth_headers, nc_borrador, con_todos_los_permisos, tmp_path
    ):
    ...
    def test_listar_adjuntos_nc(
        self, client, auth_headers, nc_borrador, con_todos_los_permisos
    ):
        r = client.get(
            f"{BASE}/ncs-locales/{nc_borrador.id}/adjuntos", headers=auth_headers
        )
        assert r.status_code == 200
